...
class NettyServerStream extends AbstractServerStream {
    ...
    private class Sink implements AbstractServerStream.Sink {
        ...
        class WriteChannelFutureListener implements ChannelFutureListener
        {
            private final ObjectPool.Handle<WriteChannelFutureListener> handle;

            WriteChannelFutureListener(ObjectPool.Handle<WriteChannelFutureListener> handle){
                this.handle=handle;
            }
            int numBytes;
            int numMessages;
        ...
        private ObjectPool<WriteChannelFutureListener> wcPool = ObjectPool.newPool(WriteChannelFutureListener::new);
        ...
        private void writeFrameInternal(WritableBuffer frame, boolean flush, final int numMessages) {
            ...
            ByteBuf bytebuf;
            if(true) {
                bytebuf = ((NettyWritableBuffer) frame).bytebuf().retain().touch();
                frame.release();
            } else {
                bytebuf = ((NettyWritableBuffer) frame).bytebuf().touch();
            }
            final int numBytes = bytebuf.readableBytes();
            // Add the bytes to outbound flow control.
            onSendingBytes(numBytes);

            WriteChannelFutureListener pc = wcPool.get();
            pc.numBytes=numBytes;
            pc.numMessages=numMessages;

            writeQueue.enqueue(new SendGrpcFrameCommand(transportState(), bytebuf, false), flush).addListener(pc);
        }
