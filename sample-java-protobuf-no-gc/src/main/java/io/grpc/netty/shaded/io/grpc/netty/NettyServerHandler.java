...
import io.grpc.netty.shaded.io.grpc.netty.GrpcHttp2HeadersUtils.GrpcHttp2ServerHeadersDecoder;
import io.grpc.netty.shaded.io.grpc.netty.NettyServerStream.TransportState;
...
import static io.grpc.netty.shaded.io.grpc.netty.NettyServerBuilder.*;
import static io.grpc.netty.shaded.io.grpc.netty.Utils.*;
import static io.grpc.netty.shaded.io.netty.handler.codec.http2.DefaultHttp2LocalFlowController.DEFAULT_WINDOW_UPDATE_RATIO;
...
class NettyServerHandler extends AbstractNettyHandler {
    ...
    static NettyServerHandler newHandler(
        ...
        checkArgument(maxHeaderListSize > 0, "maxHeaderListSize must be positive: %s",
                maxHeaderListSize);
    ...
    static NettyServerHandler newHandler(
            ...
            boolean permitKeepAliveWithoutCalls,
            long permitKeepAliveTimeInNanos) {
        checkArgument(maxStreams > 0, "maxStreams must be positive: %s", maxStreams);
        checkArgument(flowControlWindow > 0, "flowControlWindow must be positive: %s",
                flowControlWindow);
        checkArgument(maxHeaderListSize > 0, "maxHeaderListSize must be positive: %s",
                maxHeaderListSize);
        checkArgument(maxMessageSize > 0, "maxMessageSize must be positive: %s",
                maxMessageSize);
